...
are **phase-locked** (CKA = 1.0) for a given anchor set. We keep searching
for the transformation until CKA reaches 1.0, then merge.

## Implementation touchpoints

Binary + vocabulary phase lock (1D -> 2D):
- `src/modelcypher/core/use_cases/merge_stages/stage_0_vocabulary.py`
  - Byte anchors (0-255) are aligned with `GramAligner`.
  - UnifiedAtlas anchors are phase-locked for vocabulary alignment.

Activation phase lock (3D+):

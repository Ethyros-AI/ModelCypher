...
- Merge projector A/B infrastructure (2026-02-27): end-to-end `projector_mode` wiring (`tikhonov` vs `binary`) through transplant stage, pipeline, merger, and `mc merge run --projector-mode`; `scripts/merge_ab_test.py` compares preserved fraction, CKA, PPL, and degeneration on identical merge inputs.
...
| **Merge pipeline end-to-end A/B closure** | MP-weighted projector is integrated, unit-tested, and CLI-exposed; binary-vs-Tikhonov merge experiment harness is implemented (`scripts/merge_ab_test.py`). | Complete and evaluate full merge A/B run artifact (`merge_ab_test.json`) to decide whether to retire binary projector mode. Known upstream blockers (variance concentration / stale profiles) still affect full merge pipeline reliability. | G4/G5 merge-closure decision pending artifact |
...
4. **Merge projector closure**: Complete `scripts/merge_ab_test.py` artifact and retire binary projector mode only if Tikhonov dominates on preserved fraction, CKA, PPL, and degeneration.

#!/usr/bin/env python3
"""Validate distance-kernel hierarchy falsifier artifact directories."""
...
def validate_run_dir(
    ...
    *,
    ...
    _validate_jsonl_file(
        file_path=run_dir / "per_head_classification.jsonl",
        errors=errors,
        files_checked=files_checked,
    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate distance-kernel hierarchy falsifier artifact directories.",
    )
    parser.add_argument(
        "--run-dir",
        type=Path,
    ...
    parser.add_argument(
        "--root",
        type=Path,
        default=Path("results/distance_kernel_hierarchy"),

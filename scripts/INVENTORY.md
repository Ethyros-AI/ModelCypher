...
|--------|---------|----------------|
| `explore_phi_trajectories.py` | Layer-wise norm tracking, comp/φ per task | `mc safety spectral-trajectory` (different method) |
...
```bash
# Layer-by-layer norm trajectory (comp/φ analysis)
poetry run python scripts/explore_phi_trajectories.py --model /path/to/model
...
|-------------|--------|-------------------|
| `mc safety spectral-trajectory` | SVD spectral entropy | `explore_phi_trajectories.py` (L2 norms) |
| `mc safety dimension-profile` | TwoNN intrinsic dimension | `hidden_state_analysis.py` (participation ratio) |
| `mc safety comp-phi` | TwoNN peak/final ratio | `explore_phi_trajectories.py` (norm peak/final) |

...
Test if gradient-guided modification works on larger models.

Key question: Does the orthogonal subspace scale with model size?
"""
...
class ScaleTestGradient:
    ...
    def run_experiment(self) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("SCALE TEST - LFM2-1.2B")
        logger.info("=" * 60)
        ...
        # Test: improve language, preserve geography+history (same as Exp 18)
        test_configs = [
            ("language", ["geography"]),
...
def main():
    ...
    try:
        ...
        logger.info("Falling back to 350M model for demonstration")
        model, tokenizer = load("/Volumes/CodeCypher/models/mlx-community/LFM2-350M-MLX-bf16")

    test = ScaleTestGradient(model, tokenizer)
    results = test.run_experiment()

    output_path = "data/experiments/scale_test_gradient.json"

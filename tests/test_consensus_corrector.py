...
"""Unit tests for consensus correction (requires backend)."""
...
class TestConsensusCorrector:
    ...
    def test_correction_moves_toward_consensus(self, any_backend):
        """Correction should move target position toward consensus."""
        corrector = ConsensusCorrector(any_backend)

        # Simple 2D example
        # Target is at (0, 0), consensus stress implies position at (5, 5)
        target_position = any_backend.array([0.0, 0.0])

        # Set up anchors at corners of a square
        target_anchors = {
            "anchor_0": any_backend.array([0.0, 0.0]),
            "anchor_1": any_backend.array([10.0, 0.0]),
            "anchor_2": any_backend.array([0.0, 10.0]),
            "anchor_3": any_backend.array([10.0, 10.0]),
        }
        ...
        import math
        d = math.sqrt(50)
        consensus_stress = any_backend.array([d, d, d, d])

        # Simple activations and weights for lstsq
        target_activations = any_backend.array([[1.0, 0.0], [0.0, 1.0]])
        target_weights = any_backend.array([[1.0, 0.0], [0.0, 1.0]])
        ...
        anchor_matrix = any_backend.stack(list(target_anchors.values()), axis=0)
        consensus_position = any_backend.mean(anchor_matrix, axis=0)
        expected_delta = consensus_position - target_position
        delta_error = result.activation_delta - expected_delta
        delta_norm = float(any_backend.sqrt(any_backend.sum(delta_error ** 2)))
        eps = _scaled_eps(any_backend, expected_delta, target_position)
        assert delta_norm <= eps

    def test_apply_correction(self, any_backend):
        """Applying correction should add delta to weights."""
        corrector = ConsensusCorrector(any_backend)

        target_weights = any_backend.array([[1.0, 2.0], [3.0, 4.0]])
        weight_delta = any_backend.array([[0.1, 0.2], [0.3, 0.4]])

        corrected = corrector.apply_correction(target_weights, weight_delta)

        eps = _scaled_eps(any_backend, target_weights, weight_delta)
    ...
    def test_zero_correction_when_at_consensus(self, any_backend):
        """No correction needed when target is already at consensus."""
        corrector = ConsensusCorrector(any_backend)

        # Target is already at (5, 5)
        target_position = any_backend.array([5.0, 5.0])
        ...
        delta_norm = float(any_backend.sqrt(any_backend.sum(result.activation_delta ** 2)))
        eps = _scaled_eps(any_backend, target_position, consensus_stress)
        assert delta_norm <= eps

    def test_stress_from_position(self, any_backend):
        """Stress computation should return correct distances."""
        corrector = ConsensusCorrector(any_backend)

        position = any_backend.array([5.0, 5.0])
        anchor_positions = {
            "anchor_0": any_backend.array([0.0, 0.0]),
            "anchor_1": any_backend.array([10.0, 10.0]),
        }
        ...
        eps = _scaled_eps(any_backend, stress, expected_d0, expected_d1)
        assert abs(float(stress[0]) - expected_d0) <= eps
        assert abs(float(stress[1]) - expected_d1) <= eps

    def test_multilateration_accuracy(self, any_backend):
        """Multilateration should recover position from distances."""
        corrector = ConsensusCorrector(any_backend)

        # Known position
        true_position = any_backend.array([3.0, 7.0])

        # Anchors
        anchor_positions = {
            "a0": any_backend.array([0.0, 0.0]),
            "a1": any_backend.array([10.0, 0.0]),
            "a2": any_backend.array([0.0, 10.0]),
            "a3": any_backend.array([10.0, 10.0]),
        }
        ...
        stress_arr = any_backend.array(stress)
        ...
        dist = float(any_backend.sqrt(any_backend.sum((recovered - true_position) ** 2)))
        eps = _scaled_eps(any_backend, recovered, true_position)
        assert dist <= eps
...
class TestCorrectionVsAddition:
    ...
    def test_correction_changes_behavior(self, any_backend):
        """Correction should change output, unlike null-space addition."""
        corrector = ConsensusCorrector(any_backend)

        # Simple weights and activations
        target_weights = any_backend.array([[1.0, 0.0], [0.0, 1.0]])
        target_activations = any_backend.array([[1.0, 0.5], [0.5, 1.0]])
        ...
        import math
        d = math.sqrt(50)
        consensus_stress = any_backend.array([d, d, d])
        ...
        delta_norm = float(any_backend.sqrt(any_backend.sum(result.weight_delta ** 2)))
        eps = _scaled_eps(any_backend, result.weight_delta, target_weights)
        assert delta_norm > eps
        ...
        original_output = any_backend.matmul(target_activations, any_backend.transpose(target_weights))
        corrected_output = any_backend.matmul(target_activations, any_backend.transpose(corrected_weights))

        output_diff = float(any_backend.sqrt(any_backend.sum((corrected_output - original_output) ** 2)))
        eps = _scaled_eps(any_backend, corrected_output, original_output)

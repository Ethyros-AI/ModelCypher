...
class TestStackedLoRAState:
    ...
    def test_adapter_count_triggers_merge(self, tmp_path: Path) -> None:
        ...
        adapters = [
            ...
            for i in range(MAX_ADAPTERS_BEFORE_MERGE)
        ]
...
class TestLoRAStacker:
    ...
    def test_new_stacker_empty(self, tmp_path: Path) -> None:
        """New stacker starts empty."""
        stacker = LoRAStacker(tmp_path / "model")
        
        assert stacker.state.n_adapters == 0
        assert stacker.state.cumulative_barrier == 0.0

    def test_add_adapter_updates_state(self, tmp_path: Path) -> None:
    ...
    def test_add_nonexistent_adapter_fails(self, tmp_path: Path) -> None:
    ...
    def test_barriers_accumulate(self, tmp_path: Path) -> None:
    ...
    def test_cka_drift_uses_max(self, tmp_path: Path) -> None:
    ...
    def test_should_merge_triggers_correctly(self, tmp_path: Path) -> None:
    ...
    def test_get_adapter_paths(self, tmp_path: Path) -> None:
    ...
    def test_save_and_resume_state(self, tmp_path: Path) -> None:
        ...
        stacker1 = LoRAStacker(base_path)
        adapter_path = tmp_path / "adapter1"
        adapter_path.mkdir()
        ...
        stacker2 = LoRAStacker(base_path, state_path=state_file)
    ...
    def test_get_status(self, tmp_path: Path) -> None:
        ...
        assert status["thresholds"]["barrier_merge"] == BARRIER_MERGE_THRESHOLD


class TestMergeEmpty:
    """Test merge with empty stack."""

    def test_merge_empty_fails(self, tmp_path: Path) -> None:

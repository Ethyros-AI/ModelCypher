...
"""Property-based tests for entropy calculations (requires backend)."""
...
def _log_scalar(backend, value: float) -> float:
...
class TestEntropyProperties:
    ...
    def test_uniform_distribution_maximum_entropy(self, logits):
        ...
        n = logits.shape[0]
        max_entropy = _log_scalar(backend, float(n))

        # Should be close to maximum
        eps = _eps(backend, entropy, max_entropy)
    ...
    def test_peaked_distribution_low_entropy(self, logits):
        ...
        eps = _eps(backend, entropy, entropy_uniform)

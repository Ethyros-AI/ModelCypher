...
class TestMCPCLIParity:
    ...
    def test_storage_status_schema_parity(self):
        """Test that mc_storage_status MCP output matches CLI storage status output schema."""
        # Import services
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                # MCP output format (from server.py mc_storage_status)
                mcp_output = {
                ...
                assert mcp_output["_schema"] == "mc.storage.status.v1"
...
def test_mcp_output_has_required_metadata(command_type: str):
    ...
    schema_patterns = {
        "storage_status": "mc.storage.status.v1",

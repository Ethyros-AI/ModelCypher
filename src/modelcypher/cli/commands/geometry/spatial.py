...
"""Geometry spatial 3D CLI commands.

Provides commands for probing spatial concept structure in LLMs.

Commands:
    mc geometry spatial anchors
    mc geometry spatial probe-model --model <path> [--layer <int>]
"""
...
def _get_spatial_probes():
    ...
    return [p for p in all_probes if p.source == AtlasSource.SPATIAL_CONCEPT]


@app.command("anchors")
def spatial_anchors(ctx: typer.Context):
    """List all spatial concept anchors used for 3D world model probing."""
    ...
    anchors = _get_spatial_probes()

    if context.output_format == "text":
        lines = ["SPATIAL CONCEPT ANCHORS", f"Total: {len(anchors)}", ""]
...
def spatial_probe_model(
    ...
    """
    Probe a model for spatial concept geometry.

    Extracts activations for spatial concept anchors and computes:
    - Intrinsic dimension of spatial subspace
    ...
    # Get spatial probes
    probes = _get_spatial_probes()
    if not probes:
        write_error(
            ErrorDetail(
                code="MC-4003",
                message="No spatial probes found",
                detail="UnifiedAtlasInventory has no SPATIAL_CONCEPT probes",
            ),
            context.output_format,
        )
        raise typer.Exit(1)
    ...
    # Extract upper triangle for mean (excluding diagonal)
    n = len(names)
    total_sim = 0.0
    count = 0
    for i in range(n):

...
"""Geometry temporal topology CLI commands.

Provides commands for probing temporal structure in LLM representations.

Commands:
    mc geometry temporal anchors
    mc geometry temporal probe-model --model <path> [--layer <int>]
"""
...
def _get_temporal_probes():
    """Get temporal probes from unified atlas."""
    ...
    return [p for p in all_probes if p.source == AtlasSource.TEMPORAL_CONCEPT]


@app.command("anchors")
def temporal_anchors(ctx: typer.Context):
    """List all temporal concept anchors used for time structure probing."""
    ...
    anchors = _get_temporal_probes()

    if context.output_format == "text":
        lines = ["TEMPORAL CONCEPT ANCHORS", f"Total: {len(anchors)}", ""]
...
def temporal_probe_model(
    ...
    """
    Probe a model for temporal concept geometry.

    Extracts activations for temporal concept anchors and computes:
    - Intrinsic dimension of temporal subspace
    ...
    # Get temporal probes
    probes = _get_temporal_probes()
    if not probes:
        write_error(
            ErrorDetail(
                code="MC-4003",
                message="No temporal probes found",
                detail="UnifiedAtlasInventory has no TEMPORAL_CONCEPT probes",
            ),
            context.output_format,
        )
        raise typer.Exit(1)

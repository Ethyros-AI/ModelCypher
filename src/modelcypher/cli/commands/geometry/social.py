...
"""Geometry social CLI commands.

Provides commands for probing social geometry in LLM representations.

Commands:
    mc geometry social anchors
    mc geometry social probe-model --model <path> [--layer <int>]
"""
...
def _get_social_probes():
    """Get social probes from unified atlas."""
    ...
    return [p for p in all_probes if p.source == AtlasSource.SOCIAL_CONCEPT]


@app.command("anchors")
def social_anchors(ctx: typer.Context):
    """List all social concept anchors used for social geometry probing."""
    ...
    anchors = _get_social_probes()

    if context.output_format == "text":
        lines = ["SOCIAL CONCEPT ANCHORS", f"Total: {len(anchors)}", ""]
...
def social_probe_model(
    ...
    Extracts activations for social concept anchors and computes:
    - Intrinsic dimension of social subspace
    ...
    # Get probes
    probes = _get_social_probes()
    if not probes:
        write_error(
            ErrorDetail(
                code="MC-4003",
                message="No social probes found",
                detail="UnifiedAtlasInventory has no SOCIAL_CONCEPT probes",
            ),
            context.output_format,
        )
        raise typer.Exit(1)

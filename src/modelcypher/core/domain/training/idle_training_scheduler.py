...
class IdleTrainingScheduler:
    ...
    async def evaluate(self):
        ...
        try:
            ...
            if not self.policy.enabled:
                ...
                await self._resume_managed_jobs_if_possible(
                    paused_jobs, running_jobs, status, force=True
                )
                return
            ...
            status = SchedulerStatus(thermal_raw, memory_critical, now)
            await self._resume_managed_jobs_if_possible(
                paused_jobs, running_jobs, status, force=False
            )
    ...
    async def _pause_jobs_if_needed(
        self, running_jobs: list[JobSummary], reason: PauseReason, thermal_raw: int, now: float
    ):
        if not self.training_service:
            return
        ...
        for job in running_jobs:
            if job.id not in self.managed_jobs:
                try:
                    ...
                    self._mark_state_dirty()
                    logger.info(
                        f"Paused job {job.id} due to {reason.value} (thermal: {thermal_raw})"
                    )
                except Exception as e:
    ...
    async def _resume_managed_jobs_if_possible(
        ...
        if not force and (
            status.thermal_raw > self.policy.max_thermal_state_raw or status.memory_critical
        ):
            return
        ...
        idle_duration = (
            (status.timestamp - self.last_idle_transition) if self.last_idle_transition else 0.0
        )
        resumed_jobs = []
        ...
        for job_id in managed_ids:
            ...
            if not force:
                if idle_duration < self.policy.min_idle_seconds:
                    continue
                if (status.timestamp - managed.paused_at) < self.policy.min_idle_seconds:
                    continue

...
class ContinualLearningMetrics:
    ...
    def cka_stability(cka_history: list[list[float]]) -> dict[str, float | None]:
        ...
        return {
            "min": min(prior_evals),
            "mean": sum(prior_evals) / float(len(prior_evals))
        }

    @staticmethod
    def standard_cl_metrics(accuracy_matrix: list[list[float]], random_baselines: list[float] | None = None) -> StandardCLSummary:

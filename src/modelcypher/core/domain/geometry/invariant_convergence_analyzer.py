...
class InvariantConvergenceAnalyzer:
    ...
    def analyze(
        ...
        for family in ordered_families:
            anchor_ids = [aid for aid in source_vectors.keys() if aid.startswith(f"{family}_")]
        ...
        mean_by_layer = {
            self._format_layer_label(layer, align_mode, axis_by_index): self._mean_across_families(
                layer, family_results, align_mode, axis_by_index
            )
            for layer in layers_union
        }

        output_layers = [
            axis_by_index.get(layer, float(layer))
            if align_mode == AlignMode.NORMALIZED
            else float(layer)
            for layer in layers_union
        ]
        ...
        return Report(
            ...
            layer_count=min(src_count, tgt_count)
            if src_count > 0 and tgt_count > 0
            else max(src_count, tgt_count),

...
class SequenceProbeResult:
    ...
    Sequences are generated by hybrid vocab + gradient optimization:
    1. Score vocabulary tokens for null-space activation (closed-form)
    2. Take top-k tokens as seeds
    3. Optimize multi-token sequences via gradient ascent
    4. Discretize to nearest token sequences
    """
    ...
    gradient_steps: int  # Number of optimization steps taken
    improvement_ratio: float  # Final null_space_norm / initial null_space_norm
...
def optimize_sequence_for_null_space(
    seed_embeddings: "Array",
    null_basis: "Array",
    model_inner: Any,
    layer_idx: int,
    backend: "Backend",
    ...
    """Optimize embedding sequence to maximize null-space activation.

    This optimizes the full sequence jointly, respecting attention/RoPE
    interactions between positions. The loss is:

        L = -||U_null^T @ activation_sequence||²

    where activation_sequence is the mean-pooled activation over positions.

    Args:
        seed_embeddings: Initial embeddings [seq_len, embed_dim] from vocab tokens.
        null_basis: Null space basis [hidden_dim, null_rank] from SVD.
        model_inner: Inner model (has .layers attribute).
        layer_idx: Target layer index for activation.
        backend: Backend for tensor operations.
        seq_len: Length of sequence to optimize.
        steps: Number of gradient ascent steps.
        lr: Learning rate for gradient ascent.

    Returns:
        Tuple (optimized_embeddings, initial_norm, final_norm) where:
        - optimized_embeddings: [seq_len, embed_dim] after optimization
        - initial_norm: ||U_null^T @ activation|| before optimization
        - final_norm: ||U_null^T @ activation|| after optimization
    """
    b = backend
    embeds = _promote_precision(seed_embeddings, b)
    U_null = _promote_precision(null_basis, b)
    b.eval(embeds, U_null)

    initial_norm = 0.0
    final_norm = 0.0

    # Forward pass helper - compute activation at target layer
    def forward_to_layer(e: "Array") -> "Array":
        """Forward through layers to get activation."""
        h = b.expand_dims(e, axis=0)  # [1, seq_len, embed_dim]
        ...
        # Mean pool over batch and sequence: [hidden_dim]
        activation = b.mean(h, axis=(0, 1))
        return activation

    # Compute null-space projection norm
    def null_space_norm(activation: "Array") -> "Array":
        """Compute ||U_null^T @ activation||."""
        proj = b.matmul(b.transpose(U_null), activation)  # [null_rank]
        norm_sq = b.sum(proj * proj)
        return b.sqrt(norm_sq + 1e-8)

    # Objective: negative norm (we want to maximize)
    def objective(e: "Array") -> "Array":
        activation = forward_to_layer(e)
        return -null_space_norm(activation)

    # Compute initial norm
    init_activation = forward_to_layer(embeds)
    b.eval(init_activation)
    init_norm_arr = null_space_norm(init_activation)
    b.eval(init_norm_arr)
    initial_norm = float(b.to_scalar(init_norm_arr))
    ...
    for step in range(steps):
        ...
        # Gradient descent on negative norm = gradient ascent on norm
        embeds = embeds - lr * grad
        b.eval(embeds)

        if step % 5 == 0:
            logger.debug(
                "Step %d/%d: null_proj_norm = %.4f",
                step, steps, -float(b.to_scalar(loss))
            )

    # Compute final norm
    final_activation = forward_to_layer(embeds)
    b.eval(final_activation)
    final_norm_arr = null_space_norm(final_activation)
    b.eval(final_norm_arr)
    final_norm = float(b.to_scalar(final_norm_arr))

    return embeds, initial_norm, final_norm


def discretize_embeddings_to_tokens(
    embeddings: "Array",
    embedding_matrix: "Array",
    backend: "Backend",
    soft_discretize: bool = True,
    temperature: float = 0.1,
) -> list[int]:
    """Discretize continuous embeddings to nearest token IDs.

    Args:
        embeddings: Optimized embeddings [seq_len, embed_dim].
        embedding_matrix: Vocabulary embedding matrix [vocab_size, embed_dim].
        backend: Backend for tensor operations.
        soft_discretize: If True, use softmax-weighted selection (more diverse).
                        If False, use hard argmin (nearest neighbor).
        temperature: Temperature for softmax (lower = sharper, more greedy).

    Returns:
        List of token IDs [seq_len].
    """
    b = backend
    embeds = _promote_precision(embeddings, b)
    embed_weight = _promote_precision(embedding_matrix, b)
    b.eval(embeds, embed_weight)

    seq_len = int(b.shape(embeds)[0])
    token_ids: list[int] = []
    ...
    for pos in range(seq_len):
        ...
        # Compute distances to all vocab embeddings
        # diff[i] = embed_weight[i] - embed_pos
        diff = embed_weight - b.expand_dims(embed_pos, axis=0)
        dists_sq = b.sum(diff * diff, axis=1)  # [vocab_size]
        b.eval(dists_sq)

        if soft_discretize:
            # Soft selection via Gumbel-softmax style (without actual Gumbel noise)
            # Convert distances to similarities (negative distances)
            neg_dists = -dists_sq / temperature
            b.eval(neg_dists)

            # Softmax to get probabilities
            max_val = b.max(neg_dists)
            b.eval(max_val)
            exp_dists = b.exp(neg_dists - max_val)
            b.eval(exp_dists)
            probs = exp_dists / b.sum(exp_dists)
            b.eval(probs)

            # Sample or take max
            token_id = int(b.to_scalar(b.argmax(probs)))
        else:
            # Hard discretization: nearest neighbor
            token_id = int(b.to_scalar(b.argmin(dists_sq)))
...
def find_null_space_sequences(
    ...
    Hybrid approach combining closed-form vocabulary scoring with gradient
    optimization:

    1. **Vocabulary scan** (closed-form): Score all vocab tokens by
       ||U_null^T @ activation|| to find tokens that already activate
       null-space directions.

    2. **Gradient refinement**: Starting from top-k vocab tokens, optimize
       multi-token sequences via gradient ascent on ||U_null^T @ activation||².

    3. **Discretization**: Convert optimized embeddings back to token IDs
       using soft nearest-neighbor.

    This is more efficient than pure gradient generation because:
    - Vocabulary scan is O(V) forward passes, batched
    - Gradient refinement is O(k * steps) forward passes
    - Total: O(V + k*steps) vs O(V * layers * rounds) for current approach
    ...
    Args:
        ...
        backend: Backend for tensor operations.
        seq_len: Length of sequences to generate (4-8 recommended).
        top_k_seeds: Number of top vocabulary tokens to use as seeds.
        gradient_steps: Number of gradient ascent steps per sequence.
        learning_rate: Learning rate for gradient optimization.
        batch_size: Batch size for vocabulary scanning.
    ...
    Example:
        ...
        ...     seq_len=4, top_k_seeds=25, gradient_steps=10
        ... )
    ...
    b = backend
    logger.info(
        "SEQUENCE PROBE: Finding %d-token sequences for layer %d "
        "(top_k=%d, grad_steps=%d)",
        seq_len, layer_idx, top_k_seeds, gradient_steps
    )
    ...
    if hasattr(inner, "embed_tokens"):
        ...
        logger.error("SEQUENCE PROBE: Cannot find embedding layer")
        return []

    embed_weight = _promote_precision(embed_weight, b)
    b.eval(embed_weight)
    ...
    # =========================================================================
    # PHASE 1: Closed-form vocabulary scan
    # =========================================================================
    # Find top-k tokens that already activate null-space directions.
    # This is fast (batched) and gives us good starting points.
    top_tokens = find_null_space_tokens_closed_form(
        model=model,
        U_null=U_null,
        layer_idx=layer_idx,
    ...
    logger.info(
        "SEQUENCE PROBE: Phase 1 complete - top token score=%.4f, bottom=%.4f",
        top_tokens[0][1],
        top_tokens[-1][1],
    )

    # =========================================================================
    # PHASE 2: Gradient-refined sequence generation
    # =========================================================================
    # For each seed token, generate a seq_len sequence via gradient optimization.
    results: list[SequenceProbeResult] = []

    for seed_idx, (seed_token_id, seed_score) in enumerate(top_tokens):
        # Initialize sequence: seed token + random padding
        seed_embed = b.take(embed_weight, b.array([seed_token_id], dtype="int32"), axis=0)
        seed_embed = b.squeeze(seed_embed, axis=0)  # [embed_dim]
        b.eval(seed_embed)

        # Build initial sequence embeddings
        if seq_len > 1:
            # Use seed token at first position, random tokens for rest
            init_embeds_list = [seed_embed]
            for _ in range(seq_len - 1):
                # Random token (avoid edge tokens)
                rand_id = b.array([100 + (seed_idx * 137 + _ * 17) % (vocab_size - 200)], dtype="int32")
                rand_embed = b.take(embed_weight, rand_id, axis=0)
                rand_embed = b.squeeze(rand_embed, axis=0)
                b.eval(rand_embed)
                init_embeds_list.append(rand_embed)

            init_embeds = b.stack(init_embeds_list, axis=0)  # [seq_len, embed_dim]
        else:
            init_embeds = b.expand_dims(seed_embed, axis=0)  # [1, embed_dim]
        b.eval(init_embeds)

        # Optimize sequence for null-space activation
        optimized_embeds, init_norm, final_norm = optimize_sequence_for_null_space(
            seed_embeddings=init_embeds,
            null_basis=U_null,
            model_inner=inner,
            layer_idx=layer_idx,
            backend=b,
        ...
        # Discretize to token IDs
        token_ids = discretize_embeddings_to_tokens(
            embeddings=optimized_embeds,
            embedding_matrix=embed_weight,
            backend=b,
            soft_discretize=False,  # Hard discretization for determinism
        )
        ...
        improvement_ratio = final_norm / max(init_norm, 1e-8)

        results.append(SequenceProbeResult(
            token_ids=token_ids,
            text=text,
            null_space_norm=final_norm,
        ...
        if (seed_idx + 1) % 10 == 0:
            logger.debug(
                "SEQUENCE PROBE: Generated %d/%d sequences (best norm=%.4f)",
                seed_idx + 1,
                len(top_tokens),
                max(r.null_space_norm for r in results),
            )

    # Sort by null_space_norm descending
    results.sort(key=lambda r: r.null_space_norm, reverse=True)
    ...
    if results:
        ...
        mean_improvement = sum(r.improvement_ratio for r in results) / len(results)

        logger.info(
            "SEQUENCE PROBE: Phase 2 complete - %d sequences, "
            "best=%.4f, worst=%.4f, mean=%.4f, mean_improvement=%.2fx",
            len(results),
            best.null_space_norm,
            worst.null_space_norm,
            mean_norm,
            mean_improvement,
        )
...
def compute_trajectory_tangent_null_space(
    ...
    try:
        ...
        if max_s < eps:
            # No significant velocity in null space - use minimum fraction
            tangent_rank = max(1, int(null_rank * min_tangent_fraction))
            logger.warning(
                "TRAJECTORY TANGENT: Velocities don't project into null space "
                "(max_singular=%.4e). Using min_tangent_fraction=%d dims",
                max_s, tangent_rank
            )
        else:
            threshold = max_s * threshold_factor
            ...
            # Ensure at least min_tangent_fraction
            min_rank = max(1, int(null_rank * min_tangent_fraction))
            tangent_rank = max(tangent_rank, min_rank)
...
__all__ = [
    ...
    # Sequence-based probe generation (hybrid vocab + gradient)

...
class SurpriseDetector:
    ...
    def __init__(
        ...
        activation_history_size: int = 32,
        backend: Backend | None = None,
    ) -> None:
        """Initialize the surprise detector.

        Args:
            baseline_window: Window size for computing baseline surprise.
            context_window: Number of recent tokens to include in events.
        ...
        self._activation_history_size = activation_history_size

        # Surprise history for baseline
        self._surprise_history: deque[float] = deque(maxlen=baseline_window)

        # Token history for context
        self._token_history: deque[int] = deque(maxlen=context_window)

        # Activation history for activation surprise
        self._activation_history: deque[Array] = deque(maxlen=activation_history_size)

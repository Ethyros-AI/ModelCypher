...
class RegexContentFilter:
    ...
    _pii_email_whitelist = {"example.com", "example.net", "example.org", "test.com", "localhost"}
    ...
    def _build_default_rules() -> list[FilterRule]:
        ...
        return [
            ...
            make_rule(
                ...
                SafetyCategory.DANGEROUS_CODE,
            ),
            make_rule(
                "format_disk",
                r"diskutil\s+eraseDisk",
                RuleAction.FLAG,
                "Disk erasure command",
                SafetyCategory.DANGEROUS_CODE,
            ),
            make_rule(
                "shell_commands",
                r"(?:sudo\s+)?(?:rm|chmod|chown)\b",
                RuleAction.FLAG,
            ...
            make_rule(
                ...
                r"(?:system\s*:\s*|<\|system\|>|<system>|###\s*system)",
                RuleAction.FLAG,
            ...
            make_rule(
                "prompt_jailbreak",
                r"(?:DAN\s+mode|jailbreak|act\s+as\s+(?:DAN|uncensored|unfiltered)|ignore\s+(?:all\s+)?(?:safety|ethical)\s+(?:guidelines|rules))",
                RuleAction.REJECT,
            ...
            make_rule(
                "toxicity_self_harm",
                r"(?:how\s+to\s+(?:kill|hurt)\s+(?:myself|yourself)|(?:want|going)\s+to\s+(?:die|end\s+it)|suicide\s+(?:method|instruction))",
                RuleAction.REJECT,
            ...
            make_rule(
                "toxicity_violence",
                r"(?:(?:kill|murder|attack|bomb|shoot)\s+(?:people|person|everyone|them)|(?:mass|school)\s+shooting|(?:terrorist|terror)\s+attack)",
                RuleAction.REJECT,
                "Violence/threat content detected",
                SafetyCategory.VIOLENCE,
            ),
            make_rule(
                "toxicity_hate_slurs",
                r"\b(?:n[i1]gg[ae3]r|f[a4]gg[o0]t|k[i1]ke|sp[i1]c|ch[i1]nk|wetback|towelhead)\b",
                RuleAction.REJECT,
                "Hate speech/slur detected",
                SafetyCategory.HATE_SPEECH,
            ),
            make_rule(
                "toxicity_dehumanize",
                r"(?:(?:all|those)\s+\w+\s+(?:should|must|need\s+to)\s+(?:die|be\s+killed|be\s+eliminated))",
                RuleAction.REJECT,
                "Dehumanizing content detected",
                SafetyCategory.HATE_SPEECH,
            ),
            make_rule(
                "toxicity_csam_indicators",
                r"(?:(?:child|minor|kid|underage)\s+(?:porn|sex|nude|naked)|(?:sexual|intimate)\s+(?:with|involving)\s+(?:child|minor|kid))",
                RuleAction.REJECT,
            ...
            make_rule(
                "toxicity_harassment",
                r"(?:(?:dox|doxx)(?:ing|ed)?|(?:share|post|leak)\s+(?:their|his|her)\s+(?:address|phone|personal))",
                RuleAction.REJECT,
                "Harassment/doxxing content detected",

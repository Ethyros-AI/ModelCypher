...
- numpy.savez for weight serialization (safetensors-compatible)
...
logger = logging.getLogger("modelcypher.merging.lora_adapter_merger_jax")
...
@dataclass
class LoRAModuleWeightsJAX:
    a: jnp.ndarray
    b: jnp.ndarray


@dataclass
class AdapterPayloadJAX:
    directory: Path
    base_model_id: str
    rank: int
    scale: float
    modules: Dict[str, LoRAModuleWeightsJAX]
    a_key_by_module: Dict[str, str]
    b_key_by_module: Dict[str, str]
    extra_weights: Dict[str, jnp.ndarray]


@dataclass
class MergeMatrixResultJAX:
    merged: jnp.ndarray
    conflict_count: int
    merged_non_zero: int
...
class LoRAAdapterMergerJAX:
    """
    Merges multiple LoRA adapters using TIES or DARE-TIES strategy (JAX).

    Example:
        report = LoRAAdapterMergerJAX.merge(
            adapter_directories=[Path("adapter1"), Path("adapter2")],
            output_directory=Path("merged"),
            config=ConfigJAX(strategy=StrategyJAX.TIES, ties_top_k=0.2)
        )
    """
    ...
    def merge(
        ...
        Returns:
            MergeReportJAX with statistics about the merge.
        ...
        # Load all adapters
        payloads = [LoRAAdapterMergerJAX._load_adapter(d) for d in adapter_directories]
        first = payloads[0]
        ...
        drop_rates, sparsity_summaries = LoRAAdapterMergerJAX._resolve_drop_rates(
            payloads, config
        )

        # Merge each module
        merged_weights: Dict[str, jnp.ndarray] = {}
        total_elements = 0
        merged_non_zero = 0
        conflict_count = 0
        trimmed_non_zero = 0
        ...
        for module in sorted(module_keys):
            ...
            a_result = LoRAAdapterMergerJAX._merge_matrices(
                matrices=a_matrices,
            ...
            b_result = LoRAAdapterMergerJAX._merge_matrices(
                matrices=b_matrices,
                drop_rates=drop_rates,
                config=config,
        ...
        LoRAAdapterMergerJAX._write_merged_adapter(
        ...
        avg_drop_rate = None
        if config.strategy == StrategyJAX.DARE_TIES and drop_rates:
            avg_drop_rate = sum(drop_rates) / len(drop_rates)

        return MergeReportJAX(
    ...
    @staticmethod
    def ties_merge(vectors: List[List[float]]) -> TIESMergeResultJAX:
        """
    ...
    @staticmethod
    def _load_adapter(directory: Path) -> AdapterPayloadJAX:
        ...
        if not weights_path.exists():
            ...
            weights_path = directory / "adapter_model.npz"
            if not weights_path.exists():
                raise MergeError(f"Missing adapter weights in {directory}")
        ...
        if str(weights_path).endswith(".safetensors"):
            ...
            weights = {k: jnp.array(loaded[k]) for k in loaded.files}

        # Parse LoRA weights
        modules: Dict[str, LoRAModuleWeightsJAX] = {}
        a_by_module: Dict[str, jnp.ndarray] = {}
        b_by_module: Dict[str, jnp.ndarray] = {}
        a_key_by_module: Dict[str, str] = {}
        b_key_by_module: Dict[str, str] = {}
        extra_weights: Dict[str, jnp.ndarray] = {}
        ...
        for key in module_keys:
            modules[key] = LoRAModuleWeightsJAX(a=a_by_module[key], b=b_by_module[key])

        return AdapterPayloadJAX(
            directory=directory,
    ...
    def _resolve_drop_rates(
        payloads: List[AdapterPayloadJAX],
        config: ConfigJAX,
    ) -> Tuple[List[float], List[AdapterSparsitySummaryJAX]]:
        """Determine drop rates for each adapter (DARE-TIES only)."""
        if config.strategy != StrategyJAX.DARE_TIES:
            return [0.0] * len(payloads), []

        drop_rates: List[float] = []
        summaries: List[AdapterSparsitySummaryJAX] = []
        ...
        for payload in payloads:
            ...
            for weights in payload.modules.values():
                a_flat = np.array(weights.a).flatten().tolist()
                b_flat = np.array(weights.b).flatten().tolist()
    ...
    def _merge_matrices(
        ...
        vectors = [np.array(m).astype(np.float32).flatten().tolist() for m in matrices]
        ...
        if config.strategy == StrategyJAX.DARE_TIES:
            for i in range(len(vectors)):
                ...
                if drop_rate > 0:
                    stable_seed = LoRAAdapterMergerJAX._stable_seed(seed, module, i)
                    rng = SeededGeneratorJAX(stable_seed)
                    vectors[i] = LoRAAdapterMergerJAX.apply_dare_drop(
        ...
        for vector in vectors:
            trimmed, kept = LoRAAdapterMergerJAX.trim_vector(vector, config.ties_top_k)
            trimmed_vectors.append(trimmed)
            trimmed_non_zero += kept
            trimmed_total += len(trimmed)

        # TIES merge
        merge_result = LoRAAdapterMergerJAX.ties_merge(trimmed_vectors)

        # Reshape back to original shape
        merged = jnp.array(merge_result.merged, dtype=dtype).reshape(shape)

        return MergeMatrixResultJAX(
            merged=merged,
    ...
    def _write_merged_adapter(
        ...
        try:
            ...
            output_weights = output_directory / "adapter_model.npz"
            np.savez(str(output_weights), **weights_np)

...
from modelcypher.core.domain.geometry.types import BatchMergerResult, MergerConfig, MergerResult
...
class TransportGuidedMerger:
    ...
    async def _merge_single(
        ...
        T_plan, dist, iters = GromovWassersteinSolver.solve(
            C1, C2, p, q, epsilon=config.gw_config.epsilon
        )
        # T_plan is [Ns, Nt]
            ...
            if k in target_weights and k in source_activations and k in target_activations:
                try:
                    res = await TransportGuidedMerger._merge_single(
                        source_weights[k],
                        target_weights[k],
                        source_activations[k],
                        target_activations[k],
                        config,
                    )
                    results[k] = res

...
def compute_token_rank_metrics_cuda(
    ...
    Args:
        scores: 1D tensor of logit scores.
    ...
    token_score = scores[token_id].item()

    # Rank = count of tokens with strictly higher score.
    token_rank = int((scores > token_score).sum().item())
    ...
    if vocab_size > 1:
        ...
        if max_gap <= eps:
            ...
            frontier_size = int(torch.argmax(gaps).item()) + 1
    else:
        frontier_size = 1
    ...
    async def generate(self, prompt: str) -> AsyncGenerator[dict[str, Any], None]:
        ...
        with torch.no_grad():
            ...
            while token_count < self.max_tokens:
                ...
                base_entropy, base_variance = compute_entropy_cuda(logits_base[0])
                adapter_entropy, adapter_variance = compute_entropy_cuda(logits_adapter[0])

                # Compute KL divergence
                kl_div = compute_kl_divergence_cuda(logits_adapter[0], logits_base[0])

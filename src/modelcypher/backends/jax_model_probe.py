...
class JAXModelProbe(BaseModelProbe):
    ...
    def __init__(self) -> None:
        """Initialize JAX probe, checking for JAX availability."""
        try:
            import jax
    ...
    def _analyze_weights(self, model_path: Path) -> tuple[list[LayerInfo], int]:
        """Analyze weight files using JAX/safetensors."""
        ...
        for st_file in safetensor_files:
            try:
                # Use flax framework for JAX-compatible tensors with bfloat16 support
    ...
    def _load_weight_tensors(self, model_path: Path) -> dict[str, Any]:
        """Load weight tensors using JAX (supports bfloat16)."""
        from safetensors import safe_open
        ...
        for st_file in safetensor_files:
            try:
                # framework="flax" returns JAX arrays with bfloat16 support
                with safe_open(st_file, framework="flax") as f:
                    for key in f.keys():

...
"""Value Geometry: Measuring value-related structure in representations.

Analyzes how value-laden concepts (good/bad, virtue/vice, self/other) are
organized in embedding space. Computes:

1. Axis orthogonality: Independence of valence/agency/scope axes
2. Gradient consistency: Monotonicity of orderings (vice < neutral < virtue)
3. Foundation clustering: Within vs between-cluster similarity
4. Value opposition: Distance between opposing concept pairs
...
- Clustering: within-foundation / between-foundation similarity ratio
- Opposition: 1 - cosine similarity between paired concepts
"""
...
@dataclass
class ValueAxisOrthogonality:
    """Orthogonality measurements between value axes.
...
@dataclass
class ValueGradientConsistency:
    """Gradient consistency for each value axis.
...
@dataclass
class ValueFoundationClustering:
    """Clustering quality of value foundations.

    Measures how tightly concepts cluster by foundation (e.g., care/harm,
    fairness, loyalty) vs how similar concepts are across foundations.
    """

    within_foundation_similarity: float
    between_foundation_similarity: float
    separation_ratio: float  # within / between (higher = tighter clustering)
    most_distinct_foundation: str
    most_overlapping_pair: tuple[str, str]


@dataclass
class ValueOpposition:
    """Opposition structure between paired concepts.

    Distance values in [0, 2] where higher = more opposed.
    Computed as 1 - cosine similarity between concept pairs.
    """

    care_harm_opposition: float
    fairness_opposition: float
    loyalty_opposition: float
    mean_opposition: float


@dataclass
class ValueGeometryComponents:
    """Raw component scores for value geometry analysis.
    ...
    """Mean orthogonality between value axes."""

    gradient_score: float
    """Mean absolute gradient correlation across axes."""

    clustering_score: float
    """Separation ratio capped at 1.0."""
...
@dataclass
class ValueGeometryReport:
    """Complete value geometry analysis report.
    ...
    layer: int
    anchors_probed: int
    axis_orthogonality: ValueAxisOrthogonality
    gradient_consistency: ValueGradientConsistency
    foundation_clustering: ValueFoundationClustering
    value_opposition: ValueOpposition
    principal_components_variance: list[float]
    components: ValueGeometryComponents
    ...
    def composite_score(self) -> float:
        ...
        return (
            ...
            + self.components.clustering_score
            + self.components.opposition_score
        ) / 4.0
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "axisOrthogonality": {
                "valenceAgency": self.axis_orthogonality.valence_agency,
                "valenceScope": self.axis_orthogonality.valence_scope,
                "agencyScope": self.axis_orthogonality.agency_scope,
                "mean": self.axis_orthogonality.mean_orthogonality,
            },
            "gradientConsistency": {
                "valence": self.gradient_consistency.valence_correlation,
                "agency": self.gradient_consistency.agency_correlation,
                "scope": self.gradient_consistency.scope_correlation,
            },
            "foundationClustering": {
                "withinSimilarity": self.foundation_clustering.within_foundation_similarity,
                "betweenSimilarity": self.foundation_clustering.between_foundation_similarity,
                "separationRatio": self.foundation_clustering.separation_ratio,
            },
            "valueOpposition": {
                "careHarm": self.value_opposition.care_harm_opposition,
                "fairness": self.value_opposition.fairness_opposition,
                "loyalty": self.value_opposition.loyalty_opposition,
                "mean": self.value_opposition.mean_opposition,
            },
            "components": {
                "orthogonality": self.components.orthogonality_score,
                "gradient": self.components.gradient_score,
                "clustering": self.components.clustering_score,
                "opposition": self.components.opposition_score,
            },
            "compositeScore": self.composite_score,
        }


class ValueGeometryAnalyzer:
    """Analyzer for value-related structure in representations.

    Measures how value concepts are organized geometrically:
    1. Extract activations for value concept probes
    ...
    5. Detect value opposition structure
    ...
    def __init__(
        ...
        if not self._concepts:
            raise ValueError(
                ...
                "before running value geometry analysis."
            )
    ...
    def analyze(
        ...
        """Run value geometry analysis.

        Args:
            activations: Dict mapping concept ID or name to activation vector
            model_path: Path to model (for reporting)
            layer: Layer analyzed (for reporting)

        Returns:
            ValueGeometryReport with all measurements
        """
        ...
        opposition = self._compute_value_opposition(matrix_norm, concepts)
    ...
    def _compute_axis_orthogonality(
        ...
        """Compute orthogonality between value axes."""
        backend = self._backend
        valence_vecs = []
        agency_vecs = []
        scope_vecs = []
    ...
    def _compute_gradient_consistency(
        ...
        return ValueGradientConsistency(
    ...
    def _compute_foundation_clustering(
        ...
        """Analyze how well value foundations cluster."""
        backend = self._backend
        backend.eval(matrix)
        ...
        return ValueFoundationClustering(
    ...
    def _compute_value_opposition(
        self, matrix: "Array", concepts: list[str]
    ) -> ValueOpposition:
        """Compute opposition structure between paired concepts."""
        backend = self._backend
        backend.eval(matrix)
        ...
        def opposition_distance(virtue_id: str, vice_id: str) -> float:
            """Compute normalized distance (1 - cosine) between concepts."""
